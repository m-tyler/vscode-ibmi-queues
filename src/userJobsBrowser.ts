...
export function initializeUserJobBrowser(context: vscode.ExtensionContext) {
  ...
  vscode.workspace.onDidChangeConfiguration(e => {
    if (e.affectsConfiguration('vscode-ibmi-queues.userJobBrowser.userJobBrowser.enabled')) {
    ...
    context.subscriptions.push(
      ...
      vscode.commands.registerCommand(`vscode-ibmi-queues.userJobBrowser.sort`, async (node) => {
        ...
        try {
          ...
          await updateFilterConfigForServer<IBMiUserJobsUsersConfig>('userJobBrowser',config.name,userJobsConfig);
      ...
      vscode.commands.registerCommand(`vscode-ibmi-queues.userJobBrowser.ReplyToJobMessage`, async (node) => {
          ...
          if (item.activeJobStatus && item.activeJobStatus === 'MSGW') {
            ...
            if (userReply) {
              await vscode.window.withProgress({
                ...
                const msgwJobs = await IBMiContentJobs.getJobMessageWaitMessages(`vscode-ibmi-queues.userJobBrowser.ReplyToUnanswered`, [item.jobName || '']);
      ...
      vscode.commands.registerCommand(`vscode-ibmi-queues.userJobBrowser.HoldJob`, async (node) => {
        const item = node as IBMiUserJob;
        try {
...
function updateExtensionStatus(): boolean {
  ...
  const enabled = config.get<boolean>('vscode-ibmi-queues.userJobBrowser.userJobBrowser.enabled', true);
